...
| Phase 3：老师端效率增强 | 已完成基础闭环 | 已落地作业中心、设置页、收费与账户页重构；反馈/进度已进入学生详情，同时保留一级入口用于集中复盘；学生端基础承接已补齐首页和账户明细 |
...
- 学生端新版基础承接：首页聚合余额风险、最近课程、待完成作业、最新反馈；账户明细页展示余额、最近扣费和最近收款
- 学生端 `/api/billing/my/account` 只读账户接口与回归测试
- 学生端路由级懒加载和构建拆包：移除单个约 1.2 MB 入口包

### 0.4 待继续项

- 本轮主链路开发、自动化验证和本地真实浏览器冒烟已完成。
- 若继续做性能优化，可进一步分析首屏依赖和按页拆包。
- 正式发布、生产迁移、微信小程序原生验收或更完整 E2E 自动化另开专项。
