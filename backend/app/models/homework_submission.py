...
class HomeworkSubmission(db.Model):
    ...
    file_ids = db.Column(db.Text)  # JSON格式存储关联的文件ID列表
    score = db.Column(db.Integer)  # 评分
    feedback = db.Column(db.Text)  # 教师反馈
    status = db.Column(
        db.Enum("draft", "submitted", "graded", "returned", name="submission_status"),
        default="draft",
        nullable=False,
    )
    ...
    student = db.relationship(
        "User", backref="homework_submissions", foreign_keys=[student_id]
    )
    grader = db.relationship(
        "User", backref="graded_submissions", foreign_keys=[graded_by]
    )

    # 唯一约束：同一学生对同一作业只能有一个提交
    __table_args__ = (
        db.UniqueConstraint(
            "homework_id", "student_id", name="unique_homework_student"
        ),
    )
