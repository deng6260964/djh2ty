...
class Question(db.Model):
    ...
    question_bank_id = db.Column(
        db.Integer, db.ForeignKey("question_banks.id"), nullable=False
    )
    ...
    difficulty_level = db.Column(
        db.Enum(
            "beginner", "intermediate", "advanced", name="question_difficulty_levels"
        ),
        nullable=False,
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "question_bank_name": self.question_bank.name
    ...
    def check_answer(self, user_answer):
        ...
        if self.question_type in ["multiple_choice", "true_false"]:
            return (
                ...
                == str(self.correct_answer).strip().lower()
            )
        elif self.question_type == "fill_blank":
            # 填空题可能有多个正确答案
            correct_answers = [
                ans.strip().lower() for ans in self.correct_answer.split("|")
            ]
