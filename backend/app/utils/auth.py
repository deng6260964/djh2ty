...
class PasswordManager:
    ...
    def validate_password_strength(self, password: str) -> Dict:
        ...
        if self.config['require_special_chars']:
            ...
            if not re.search(special_chars_pattern, password):
                errors.append(f"Password must contain at least one special character from: {self.config['special_chars']}")
        
        # 检查常见弱密码模式
        weak_patterns = [
            (r'(.)\1{2,}', "Password should not contain repeated characters"),
            (r'123456|password|qwerty|admin', "Password should not contain common weak patterns"),
    ...
    def generate_password_requirements(self) -> Dict:
        ...
        requirements.append(f"Length: {self.config['min_length']}-{self.config['max_length']} characters")
