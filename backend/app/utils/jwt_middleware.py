...
class JWTMiddleware:
    ...
    def init_app(self, app):
        ...
        def expired_token_callback(jwt_header, jwt_payload):
            logger.warning(
                f"Expired token access attempt from user {jwt_payload.get('sub')}"
            )
            return (
                jsonify(
                    {
        ...
        def revoked_token_callback(jwt_header, jwt_payload):
            logger.warning(
                f"Revoked token access attempt from user {jwt_payload.get('sub')}"
            )
            return (
                jsonify(
    ...
    def refresh_access_token(self, current_user_id):
        ...
        try:
            ...
            logger.error(
                f"Error refreshing token for user ID {current_user_id}: {str(e)}"
            )
            raise
