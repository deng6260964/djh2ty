...
class SessionManager:
    ...
    def create_session(self, user: User, jti: str) -> SessionInfo:
        ...
        try:
            ...
            logger.info(f"Session created for user {user.email} (ID: {user.id}) from {ip_address}")
    ...
    def revoke_user_sessions(self, user_id: int, exclude_session: Optional[str] = None) -> int:
        """撤销用户的所有会话（可排除指定会话）"""
        try:
            revoked_count = 0
            sessions_to_remove = []
    ...
    def get_session_statistics(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'average_sessions_per_user': total_sessions / len(user_session_counts) if user_session_counts else 0,
    ...
    def _cleanup_user_sessions(self, user_id: int) -> None:
        ...
        for session_id, session_info in self.active_sessions.items():
            if (session_info.user_id == user_id and 
                self._is_session_expired(session_info)):
                sessions_to_remove.append(session_id)
    ...
    def _enforce_session_limit(self, user_id: int) -> None:
        """强制执行用户会话数量限制"""
        user_sessions = [(sid, sinfo) for sid, sinfo in self.active_sessions.items() 
                        if sinfo.user_id == user_id]
        
        if len(user_sessions) >= self.max_sessions_per_user:
            # 按登录时间排序，移除最旧的会话
            user_sessions.sort(key=lambda x: x[1].login_time)
            sessions_to_remove = user_sessions[:len(user_sessions) - self.max_sessions_per_user + 1]
