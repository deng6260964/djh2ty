...
def update_user_role(user_id, current_user):
    ...
    try:
        ...
        logger.info(
            f"User role updated: {user.email} from {old_role} to {new_role} by {current_user.email}"
        )
...
def validate_phone():
    ...
    try:
        ...
        if not is_valid_format:
            validation_result["suggestions"].append(
                "Please enter a valid Chinese mobile phone number (11 digits starting with 1)"
            )
