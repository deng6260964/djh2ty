...
def login():
    ...
    try:
        ...
        if not user.is_active:
            ...
            return (
                jsonify({"error": "Account is inactive", "code": "ACCOUNT_INACTIVE"}),
                401,
            )
        ...
        # 创建会话
        session_info = session_manager.create_session(user=user, jti=jti)
...
def register():
    ...
    try:
        ...
        if not re.match(email_pattern, data["email"]):
            return (
                jsonify({"error": "Invalid email format", "code": "INVALID_EMAIL"}),
                400,
            )
        ...
        if User.query.filter_by(email=data["email"]).first():
            return (
                jsonify({"error": "Email already exists", "code": "EMAIL_EXISTS"}),
                409,
            )
        ...
        logger.info(
            f"New user registered and logged in: {user.email} with role {user.role}"
        )
...
def refresh():
    ...
    try:
        ...
        if not user_sessions:
            return jsonify({"error": "NO_ACTIVE_SESSION", "message": "没有找到活跃会话"}), 401
...
def logout_all():
    ...
    try:
        ...
        revoked_count = session_manager.revoke_user_sessions(
            current_user_id, exclude_session=current_session_id
        )
...
def get_user_sessions(current_user):
    ...
    try:
        ...
        return (
            jsonify({"sessions": sessions_data, "total_count": len(sessions_data)}),
            200,
        )
...
def revoke_session(current_user, session_id):
    ...
    try:
        ...
        if not session_info:
            return (
                jsonify({"error": "Session not found", "code": "SESSION_NOT_FOUND"}),
                404,
            )
        ...
        if session_info.user_id != current_user.id:
            return (
                jsonify({"error": "Permission denied", "code": "PERMISSION_DENIED"}),
                403,
            )
        ...
        if success:
            ...
            return (
                jsonify({"error": "Failed to revoke session", "code": "REVOKE_FAILED"}),
                500,
            )
    ...
    try:
        ...
        if data.get("logout_other_sessions", False):
            ...
            session_manager.revoke_user_sessions(
                current_user.id, exclude_session=current_session_id
            )
...
def validate_token(current_user):
    ...
    try:
        ...
        if is_valid:
            ...
            return (
                jsonify({"valid": False, "message": "Token is invalid or expired"}),
                401,
            )
